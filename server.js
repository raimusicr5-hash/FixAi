...
async function gerarRespostaGemini(config) {
    ...
    for (let tentativa = 1; tentativa <= tentativas; tentativa++) {
        try {
            ...
            console.error("Erro na tentativa " + tentativa + ":", error.message || error);

            const status = error?.status || error?.error?.code;

            if (status === 503 && tentativa < tentativas) {
                const espera = tentativa * 2000;
                console.log("Gemini ocupado. Nova tentativa em " + (espera / 1000) + " segundos...");
    ...
    try {
        ...
        if (!problema && !imagem) {
            return res.status(400).json({
                erro: "Digite uma mensagem ou envie uma imagem."
            });
        }
...
`;
        ...
        const partes = [];
        partes.push({ text: instrucoes });

        if (Array.isArray(historico) && historico.length > 0) {
            const historicoTexto = historico
                .map(mensagem => {
                    const nome = mensagem.role === "user" ? "Usuário" : "FixAI";
                    return nome + ": " + mensagem.text;
                })
                .join("\n\n");
        ...
        const response = await gerarRespostaGemini({
        ...
        if (status === 503) {
            return res.status(503).json({
                erro: "A inteligência artificial está temporariamente ocupada. Tente novamente em alguns segundos."
            });
        }
